...
export function QuotePicker({ projectId, onDone }: QuotePickerProps) {
  const [search, setSearch] = useState("");

  const quotesQuery = trpc.projects.searchQuotes.useQuery(
    { search: search || undefined },
    { enabled: true }
  );

  const utils = trpc.useUtils();
  const addQuote = trpc.projects.addQuote.useMutation({
    onSuccess: () => {
    ...
    <div className="rounded-lg border" style={{ backgroundColor: COLORS.surface, borderColor: COLORS.cardBorder }}>
      <div className="flex items-center gap-2 px-3">
        ...
        <input
          ...
          placeholder="Search unassigned quotes..."
      ...
      <div className="max-h-48 overflow-y-auto border-t" style={{ borderColor: COLORS.cardBorder }}>
        {(quotesQuery.data ?? []).map((quote) => (
          <button
            key={quote.id}
            onClick={() => addQuote.mutate({ projectId, quoteId: quote.id })}
          ...
          >
            <span className="font-mono text-xs" style={{ color: COLORS.textMuted }}>{quote.number}</span>
            <span className="flex-1 truncate">{quote.title}</span>
            <span className="text-xs">{quote.status}</span>
          </button>
        ))}
        {(quotesQuery.data ?? []).length === 0 && !quotesQuery.isLoading && (
